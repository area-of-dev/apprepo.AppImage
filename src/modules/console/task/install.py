...
class InstallPackageTask(object):
    ...
    def process(self, string=None, replace=False, systemwide=False):
        ...
        if replace is None or not replace and os.path.exists(destination):
            raise Exception('{} already exists, use --force to override the existing package'.format(destination))
