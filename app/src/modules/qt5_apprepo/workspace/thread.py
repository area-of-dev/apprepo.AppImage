...
class WorkspaceThread(QtCore.QThread):
    ...
    @hexdi.inject('apprepo')
    def _search_packages(self, apprepo):
        self.packageCleanAction.emit(None)
        for entity in apprepo.search(self.string):
            self.packageAction.emit(entity)
